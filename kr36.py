...
class Crawler:
    ...
    def __init__(self, url):
        ...
        chromedriver_path = 'venv\\Lib\\site-packages\\selenium\\chromedriver.exe'
    ...
    def getUser(self, url):
        ...
        try:
            for j in range(1):
                try:
                    button = self.browser.execute_script(
                        "var div = document.getElementsByClassName('kr-loading-more-button show'); div[0].click();")
            ...
            if bs is not None:
                author_element = bs.find('a', class_="author-name")
                if author_element is not None:
                    username = author_element.get_text()
                else:
                    username = "unknown"
    ...
    def getPage(self, url):
        ...
        try:
            ...
            bs = BeautifulSoup(self.browser.page_source, 'html.parser')
            if bs is not None:
                username_tag = bs.find('a', class_="author-name")
                username = username_tag.get_text() if username_tag is not None else None
    ...
    def getEssay(self, url, user):
        ...
        for i in content_body:
            if i.xpath(".//img"):
                img_tab = "\n<img>@img_id={}</img>\n".format(int(item['img_info'][img_num]["id"]))
...
if __name__ == "__main__":
    ...
    try:
        ...
        //gpt优化代码，但会触发waf直接被ban，还得研究下怎么优化
